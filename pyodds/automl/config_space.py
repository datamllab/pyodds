...
def construct_search_space(n_samples, num_features):
	...
	contamination = hp.choice('contamination', [0.005, 0.01, 0.001])  # 0.005
	...
	space_config = hp.choice('classifier_type', [
		{
			...
			'n_estimators': hp.choice('estimators', [100, 50, 150]),  # 100
			'max_samples': "auto",
			'max_features': 1.,
		...
		{
			'type': 'lstm_ed',
			'contamination': contamination, 'num_epochs': num_epochs,
			'batch_size': hp.choice('batch_size', [20, 50, 100]),  # 20
			'lr': 1e-3,
			'hidden_size': 5, 'sequence_length': 30,
			'train_gaussian_percentage': 0.25,
		}
...
def construct_classifier(x):
	clf = None
	if x['type'] == 'iforest':
		clf = IFOREST(contamination=x['contamination'],
		...
		clf = OCSVM(gamma=x['gamma'], kernel=x['kernel'], degree=x['degree'],
		            coef0=x['coef0'], tol=x['tol'], nu=x['nu'],
		            shrinking=x['shrinking'], cache_size=x['cache_size'],
		            verbose=x['verbose'], max_iter=x['max_iter'])
	elif x['type'] == 'lof':
		clf = LOF(contamination=x['contamination'], n_neighbors=x['n_neighbors'],
		          algorithm=x['algorithm'],
		          leaf_size=x['leaf_size'], metric=x['metric'], p=x['p'],
		          metric_params=x['metric_params'],
		          novelty=True)
	elif x['type'] == 'robustcovariance':
		clf = RCOV(random_state=x['random_state'],
		           store_precision=x['store_precision'],
		           assume_centered=x['assume_centered'],
		           support_fraction=x['support_fraction'],
		...
		clf = StaticAutoEncoder(contamination=x['contamination'], epoch=x['epoch'],
		                        ...
		                        regularizer_weight=x['regularizer_weight'],
		                        activation=x['activation'],
		                        kernel_regularizer=x['kernel_regularizer'],
		                        loss_function=x['loss_function'],
		                        optimizer=x['optimizer'])
	elif x['type'] == 'cblof':
		clf = CBLOF(contamination=x['contamination'], n_clusters=x['n_clusters'],
		            clustering_estimator=x['clustering_estimator'], alpha=x['alpha'],
		            beta=x['beta'], use_weights=x['use_weights'],
		            random_state=x['random_state'], n_jobs=x['n_jobs'])
	elif x['type'] == 'knn':
		clf = KNN(contamination=x['contamination'], n_neighbors=x['n_neighbors'],
		          method=x['method'],
		          radius=x['radius'], algorithm=x['algorithm'],
		          leaf_size=x['leaf_size'], metric=x['metric'], p=x['p'],
		          metric_params=x['metric_params'], n_jobs=x['n_jobs'])
	elif x['type'] == 'hbos':
		clf = HBOS(contamination=x['contamination'], n_bins=x['n_bins'],
		           alpha=x['alpha'], tol=x['tol'])
	elif x['type'] == 'sod':
		clf = SOD(contamination=x['contamination'], n_neighbors=x['n_neighbors'],
		          ref_set=x['ref_set'], alpha=x['alpha'])
	elif x['type'] == 'pca':
		clf = PCA(contamination=x['contamination'], n_components=x['n_components'],
		          n_selected_components=x['n_selected_components'], copy=x['copy'],
		          whiten=x['whiten'],
		          svd_solver=x['svd_solver'], tol=x['tol'],
		          iterated_power=x['iterated_power'],
		          random_state=x['random_state'], weighted=x['weighted'],
		          standardization=x['standardization'])
	elif x['type'] == 'dagmm':
		clf = DAGMM(contamination=x['contamination'], num_epochs=x['num_epochs'],
		            lambda_energy=x['lambda_energy'],
		            lambda_cov_diag=x['lambda_cov_diag'],
		            lr=x['lr'], batch_size=x['batch_size'], gmm_k=x['gmm_k'],
		            normal_percentile=x['normal_percentile'],
		...
		clf = AUTOENCODER(contamination=x['contamination'],
		                  num_epochs=x['num_epochs'],
		                  batch_size=x['batch_size'], lr=x['lr'],
		                  hidden_size=x['hidden_size'],
		                  sequence_length=x['sequence_length'],
		                  train_gaussian_percentage=x['train_gaussian_percentage'])
	elif x['type'] == 'lstm_ad':
		clf = LSTMAD(contamination=x['contamination'], len_in=x['len_in'],
		             len_out=x['len_out'],
		             num_epochs=x['num_epochs'], lr=x['lr'],
		             batch_size=x['batch_size'])
	elif x['type'] == 'lstm_ed':
		clf = LSTMED(contamination=x['contamination'], num_epochs=x['num_epochs'],
		             batch_size=x['batch_size'],
		             lr=x['lr'], hidden_size=x['hidden_size'],
		             sequence_length=x['sequence_length'],
		             train_gaussian_percentage=x['train_gaussian_percentage'])
	else:
